...
<body class="cart-page">
  ...
  <button class="hamburger" aria-label="Menu"><span></span><span></span><span></span></button>
    ...
    <script>
        // Mobile nav toggle — same pattern used across every page
        document.querySelector('.hamburger').addEventListener('click', () => {
            const nav = document.querySelector('.nav-links');
            nav.style.display = nav.style.display === 'flex' ? 'none' : 'flex';
        });

        // TODO (backend): applyPromo() and checkout() are referenced by
        // the buttons above but not implemented yet. Once PHP/MySQL is
        // connected, cart items/quantities/totals should be loaded from
        // the database (or session) here instead of being hardcoded,
        // and these two functions should call the real promo-validation
